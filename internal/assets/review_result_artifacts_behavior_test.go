...
func TestReviewResultArtifactsPluginEnrichesCaptureFromProviderContext(t *testing.T) {
	...
	t.Run("findings and evidence receive provider-owned capture metadata", func(t *testing.T) {
		invocations, output := runReviewResultArtifactsHarness(t, false, false, false, false)
	...
	t.Run("binding drift fails before capture", func(t *testing.T) {
		invocations, output := runReviewResultArtifactsHarness(t, true, false, false, false)
		if len(invocations) != 2 || !containsArgument(invocations[0].Args, "--preflight") ||
	...
	t.Run("provider binding fails before launch when preflight is unavailable", func(t *testing.T) {
		invocations, output := runReviewResultArtifactsHarness(t, false, true, false, false)
	...
	t.Run("duplicate provider manifest paths fail before launch", func(t *testing.T) {
		invocations, output := runReviewResultArtifactsHarness(t, false, false, false, true)
		if len(invocations) != 1 || !containsArgument(invocations[0].Args, "--preflight") {
			t.Fatalf("duplicate manifest launched reviewer or capture: %#v", invocations)
		}
		if !strings.Contains(output, "repository_context_preflight_failed") {
			t.Fatalf("duplicate manifest error = %q", output)
		}
	})

	t.Run("capture failure preserves the provider-enriched replay payload", func(t *testing.T) {
		invocations, output := runReviewResultArtifactsHarness(t, false, false, true, false)
...
func runReviewResultArtifactsHarness(t *testing.T, drift, unsupportedPreflight, captureFail, duplicateManifest bool) ([]nativePluginInvocation, string) {
	t.Helper()
	dir := t.TempDir()
...
`), 0o600); err != nil {
	...
	manifest := []any{map[string]any{"path": "z-last.go"}, map[string]any{"path": "a-first.go"}}
	if duplicateManifest {
		manifest[1] = map[string]any{"path": "z-last.go"}
	}
