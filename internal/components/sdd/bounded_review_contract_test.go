...
var boundedReviewRequiredClauses = []string{
	...
	"START's exact provider-rendered `GENTLE_AI_REVIEW_BINDING`, including `subject_hash`",
	"reviewer returns only `findings` and `evidence`",
	"models never author hashes, manifests, or inspection metadata",
	"managed adapter derives top-level `subject_hash` and completed ordered `inspection.paths` from retained provider context",
